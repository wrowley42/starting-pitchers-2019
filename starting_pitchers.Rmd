...
stats <- stats %>%
  ...
  summarise(GS, IP, W = sum(W), ERA = mean(ERA), WHIP = mean(WHIP),.groups = "drop") 
...
cole_data <- get_pitcher_data("Gerrit Cole")

gen_break_speed_graph(cole_data)

gen_selection_graph(cole_data)

gen_outcome_graph(cole_data)

gen_comp_graph("Gerrit Cole")

```

```{r model_creation}

# creating the data for our models
...
cole_data <- get_pitcher_data("Justin Verlander")
